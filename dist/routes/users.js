...
userRouter.post("/updateMinBet/:id", async (req, res) => {
  try {
    ...
    let minUser = await MinUser.findOne({
      userId
    });
    if (minUser) {
    ...
    res.status(500).json({
      message: "Error updating minimum bet amount"
    });
  }
});
userRouter.get("/minBet/:id", async (req, res) => {
  try {
    const userId = req.params.id;
    const minUser = await MinUser.findOne({
      userId
    });
    if (!minUser) {
      return res.status(404).json({
        message: "Minimum bet amount not found"
      });
    }
    ...
    res.status(500).json({
      message: "Error fetching minimum bet amount"
    });
  }
});
